...
@bot.message_handler(func=lambda m: m.content_type == "text" and m.text == "📷 Photo ON")
def photo_on(m):
    uid = m.from_user.id
    if not is_admin(uid):
        return
...
def text_handler(m):
    ...
    try:
        ...
        if user_data[uid].get("photo_mode"):
            thumb=get_thumb(uid)
            if not thumb:
                send_message_safe(m.chat.id,"Thumb select ചെയ്തിട്ടില്ല ❌",reply_markup=main_kb())
                return
